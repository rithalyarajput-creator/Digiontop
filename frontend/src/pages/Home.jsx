...
/* The card itself is a shared component so /testimonials renders the exact
   same one — see src/components/GoogleReviewCard.jsx. */
...
const TestimonialsSection = () => {
  ...
  // Newest review first. The API orders by created_at (when it was added to the
  // admin), but what a visitor sees is reviewed_at (when it was actually left) —
  // so a genuinely older review typed in today would otherwise lead the slider.
  const display = useMemo(
    () => sortNewestFirst(items.length > 0 ? items : FALLBACK_TESTIMONIALS),
    [items],
  );
  ...
  return (
    <section className="home-testimonials" data-aos="fade-up">
      <div className="container">
        ...
        >
          {display.map((t) => (
            <GoogleReviewCard key={t.id} review={t} />
          ))}
        </div>
