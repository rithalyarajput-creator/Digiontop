...
import { FiArrowLeft, FiEye, FiUpload } from 'react-icons/fi';
  ...
  return (
    <div className="blogedit">
      <div className="admin-shop-head">
        <h1>{id ? 'Edit post' : 'New blog post'}</h1>
        <div className="admin-shop-head__actions">
          <button type="button" className="admin-sbtn" onClick={() => navigate('/admin/blog')}><FiArrowLeft /> Back</button>
          <button type="button" className={`admin-sbtn${preview ? ' admin-sbtn--primary' : ''}`} onClick={() => setPreview(v => !v)}>
      ...
      <div className="blogedit__layout">
        <form className="blogedit__form" onSubmit={handleSubmit}>
         <div className="blogedit__cols">
          <div className="blogedit__col-main">

          {/* ── BASIC INFO ── */}
          <div className="admin-card" style={{ marginBottom: 16 }}>
            <div className="admin-card__header"><h2>Basic info</h2></div>
            <div className="admin-card__body">
              ...
              <div className="admin-sfield">
                ...
                <div style={{ display: 'flex', gap: 8, alignItems: 'center' }}>
                  <span style={{ color: '#616161', fontSize: 13 }}>/blog/</span>
          ...
          {/* ── CONTENT ── */}
          <div className="admin-card" style={{ marginBottom: 16 }}>
            <div className="admin-card__header"><h2>Content</h2></div>
            <div className="admin-card__tabs">
              <button type="button" className={`admin-card__tab${editorMode === 'visual' ? ' admin-card__tab--active' : ''}`} onClick={() => setEditorMode('visual')}>Visual editor</button>
              <button type="button" className={`admin-card__tab${editorMode === 'simple' ? ' admin-card__tab--active' : ''}`} onClick={() => setEditorMode('simple')}>Text / Markdown</button>
              <button type="button" className={`admin-card__tab${editorMode === 'html' ? ' admin-card__tab--active' : ''}`} onClick={() => setEditorMode('html')}>HTML</button>
            </div>
            <div className="admin-card__body">
              {editorMode === 'visual' ? (
                <>
                ...
                <>
                  ...
                  <div className="blogedit__toolbar">
                    {[['h2','H2',true],['h3','H3',true],['strong','Bold',true],['em','Italic',true],['ul','List',true],['li','List Item',true],['a','Link',true],['img','Image',false],['blockquote','Quote',true]].map(([tag, label, wrap]) => (
                      <button key={tag} type="button" className="admin-sbtn admin-sbtn--sm" onClick={() => insertHtml(tag, wrap)} title={`Insert ${label}`}>
          ...
          <div className="admin-card">
            <div className="admin-card__header"><h2>SEO settings</h2></div>
            <div className="admin-card__body">
              <label className="admin-sfield">
                ...
                <small style={{ display: 'block', marginTop: 4, fontSize: 12, color: (form.meta_title || '').length > 60 ? '#d72c0d' : '#616161' }}>{(form.meta_title || '').length}/60 chars</small>
              </label>
              <label className="admin-sfield">
                ...
                <small style={{ display: 'block', marginTop: 4, fontSize: 12, color: (form.meta_description || '').length > 160 ? '#d72c0d' : '#616161' }}>{(form.meta_description || '').length}/160 chars</small>
              </label>
          ...
          <div className="blogedit__col-side">
            <div className="admin-card" style={{ marginBottom: 16 }}>
              <div className="admin-card__header"><h2>Status</h2></div>
              <div className="admin-card__body">
                <label className="admin-sfield" style={{ marginBottom: form.status === 'scheduled' ? undefined : 0 }}>
                ...
                {form.status === 'scheduled' && (
                  <label className="admin-sfield" style={{ marginBottom: 0 }}>
                    ...
                    />
                    <small style={{ display: 'block', marginTop: 4, fontSize: 12, color: '#616161' }}>Post will go live at this date &amp; time.</small>
                  </label>
                )}
              </div>
            </div>
            ...
            <div className="admin-card" style={{ marginBottom: 16 }}>
              ...
              <div className="admin-card__body">
                ...
                <div className="admin-supload" style={{ marginTop: 0, marginBottom: 12 }}>
                  <input
                    ref={fileRef}
                    type="file"
                    accept="image/*"
                  ...
                  <button
                    ...
                    disabled={uploading}
                  >
                    <FiUpload /> {uploading ? 'Uploading…' : 'Upload from Computer'}
                  </button>
            ...
            <div className="admin-card">
              <div className="admin-card__header"><h2>Author &amp; category</h2></div>
              <div className="admin-card__body">
                <label className="admin-sfield">
                  <span>Author</span>
          ...
          <div className="admin-card" style={{ marginTop: 16 }}>
            <div className="admin-sform__actions" style={{ borderTop: 'none' }}>
              <button type="button" className="admin-sbtn" onClick={() => navigate('/admin/blog')}>Cancel</button>
